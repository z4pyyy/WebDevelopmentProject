...
// Fetch products from categories excluding the four predefined ones
...
$query = "
  ...
  WHERE categories.name NOT IN ($placeholders)
  ORDER BY categories.name ASC, products.name ASC
";
$stmt = mysqli_prepare($conn, $query);
mysqli_stmt_bind_param($stmt, str_repeat('s', count($predefined_categories)), ...$predefined_categories);
...
<head>
  ...
  <title>Brew & Go - Other Beverages</title>
...
<body class="product-page">
  ...
  <h2 class="prd-drink-name">Other Beverages</h2>
  ...
  <div class="prd-content-container">
    <div class="search-container">
      <form class="nav-search-form" action="search.php" method="get">
        <input type="text" name="q" class="nav-search-input" placeholder="Search products..." required>
        <button type="submit" class="nav-search-btn" aria-label="Search">🔍</button>
      </form>
      <?php
      if (isset($_SESSION['search_error'])) {
          echo '<div class="search-error">' . htmlspecialchars($_SESSION['search_error']) . '</div>';
    ...
    <?php if (!empty($products_by_category)): ?>
      <?php foreach ($products_by_category as $category => $products): ?>
        ...
        <div id="<?= $category_id ?>">
          <h3 class="prd-category-title"><?= htmlspecialchars($category) ?></h3>
          <div class="prd-product-list">
            <?php foreach ($products as $row): ?>
              <?php 
              $isUnavailable = ($row['availability'] === 'Unavailable');
              // Sanitize product name for use as an ID: lowercase, replace spaces with hyphens, remove invalid characters
              $product_id = preg_replace('/[^a-z0-9-]/', '-', strtolower(str_replace(' ', '-', htmlspecialchars($row['name']))));
              ?>
              <figure id="<?= $product_id ?>" class="prd-product-item<?= $isUnavailable ? ' prd-unavailable' : '' ?>">
      ...
      <?php endforeach; ?>
    <?php else: ?>
      <p>No products available in other categories.</p>
    <?php endif; ?>
  </div>
