...
error_log("edit_product.php accessed. Session: " . print_r($_SESSION, true));
...
if (!isset($_SESSION['admin_id']) || !isset($_SESSION['role_id'])) {
    error_log("edit_product.php: Session check failed. admin_id: " . ($_SESSION['admin_id'] ?? 'not set') . ", role_id: " . ($_SESSION['role_id'] ?? 'not set'));
    header("Location: login.php");
    exit;
}

if (!checkPagePermission($conn, $currentPage, $_SESSION['role_id'])) {
    error_log("edit_product.php: Permission denied for role_id: " . $_SESSION['role_id'] . ", page: $currentPage");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_path)) {
            ...
            if (!empty($product['image_path']) && file_exists($product['image_path']) && $product['image_path'] !== $target_path) {
                unlink($product['image_path']);
            }
    ...
    // 💾 Update DB
    $stmt = mysqli_prepare($conn, "UPDATE products SET name=?, description=?, price=?, large_price=?, category_id=?, availability=?, image_path=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, "ssddissi", $name, $description, $price, $large_price, $category_id, $availability, $image_path, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
        ...
        <div class="admin-activity-thumbnail" style="text-align:center;margin-bottom:15px;">
            <label for="image" style="cursor:pointer;" title="Click to change image">
                <img src="<?= htmlspecialchars($product['image_path']) ?>" alt="Current Image"
                     style="max-width:250px;max-height:200px;border-radius:10px; transition: opacity 0.3s ease;"
                     onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">
            </label>
        </div>
    <?php endif; ?>

    <?php if (isset($_POST['delete_product'])): ?>
        <!-- Deletion Confirmation Form -->
        <div class="confirmation-form">
            <p>Are you sure you want to delete the product "<?= htmlspecialchars($product['name']) ?>"? This action cannot be undone.</p>
            <form method="POST" class="add-activity-form">
                <div class="form-buttons">
                    <button type="submit" name="confirm_delete" class="delete-edit-button">Confirm Delete</button>
                    <a href="edit_product.php?id=<?= $id ?>" class="cancel-edit-button">Cancel</a>
                </div>
            </form>
        </div>
    <?php else: ?>
        ...
        <form class="add-activity-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-buttons">
                ...
                <button type="submit" name="delete_product" class="delete-edit-button">Delete Product</button>
            </div>
        </form>
    <?php endif; ?>
</div>
