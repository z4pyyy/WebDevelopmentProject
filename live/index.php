<?php
  session_start();
?>
<!DOCTYPE html>
<html lang="en">
<head>
  
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Brew & Go Coffee - Premium handcrafted beverages">
    <meta name="keywords" content="Coffee, Brew & Go, Kuching, handcrafted beverages">
    <meta name="author" content="TERENCE WONG, DARREN CHONG, HANS YEE">
    <title>Brew & Go Coffee - Home</title>
    <link rel="stylesheet" href="styles/style.css">
    <link rel="stylesheet" href="styles/mobile.css?" media="screen and (max-width: 1300px)">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap" rel="stylesheet">
</head>

<body>
    <div id="top"></div>
    <header>
        <nav class="navbar">
            <input type="checkbox" id="nav-toggle" class="nav-toggle">
            <label for="nav-toggle" class="nav-icon">&#9776;</label>
            
            <a href="index.html" class="logo">
                <img src="images/Logo.png" alt="Brew & Go Logo">
                BREW & GO
            </a>
            
            <div class="nav-spacer"></div>

            <!-- Shopping Cart -->
            <input type="checkbox" id="cart-toggle" class="cart-toggle">
            <label for="cart-toggle" class="cart-icon">🛒</label>
            
            <div class="cart-sidebar">
                <h2>Shopping Cart</h2>
                <p>Your selected items will appear here.</p>
                <label for="cart-toggle" class="close-cart">✖ Close</label>
            </div>

            <div class="cart-overlay"></div>


            <!-- Full Navbar Menu (Hidden in Portrait) -->
            <ul class="nav-menu">
                <li><a href="index.html">Home</a></li>
                <li class="dropdown">
                  <a href="products.html">Products ▾</a>
                  <ul class="dropdown-content">
                    <li><a href="product1.html">Basic Brew</a></li>
                    <li><a href="product2.html">Artisan Brew</a></li>
                    <li><a href="product3.html">Non-Coffee</a></li>
                    <li><a href="product4.html">Seasonal</a></li>
                  </ul>
                <li class="dropdown">
                  <a href="blog.html">Blog ▾</a>
                  <ul class="dropdown-content">
                    <li><a href="#">Coming Soon</a></li>
                    <li><a href="#">Current Event</a></li>
                    <li><a href="#">Past Events</a></li>
                  </ul>
                </li>
                <li><a href="joinus.html">Join Us</a></li>
                <li><a href="enquiry.html">Enquiry</a></li>
                <li><a href="registration.html">Membership</a></li>
                <!-- <li><a href="admin.html">View</a></li>                     -->
                <li>
                  <?php if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true): ?>
                    <a href="view.php">View</a>
                  <?php else: ?>
                    <a href="login.php">Login</a>
                  <?php endif; ?>
                </li>
              </ul>
    
            <!-- Mobile Dropdown Menu -->
            <div class="nav-dropdown">
                <ul>
                    <li><a href="index.html">Home</a></li>
                    <li><a href="products.html">Products</a></li>
                    <li><a href="blog.html">Blog</a></li>
                    <li><a href="joinus.html">Join Us</a></li>
                    <li><a href="enquiry.html">Enquiry</a></li>
                    <li><a href="registration.html">Membership</a></li>
                    <li><a href="admin.html">View</a></li>                    
                </ul>
            </div>
        </nav>
    </header>
    
    <section class="hero-row">
        <!-- Section 1: BREW -->
        <div class="hero-section section-1">
          <h1 class="hero-header">BREW</h1>
        </div>
      
        <!-- Section 2: & + Button -->
        <div class="hero-section section-2">
          <div class="center-content">
            <br><br>
            <h1 class="hero-header">&</h1>
            <a href="products.html" class="btn">LEARN MORE</a>
          </div>
        </div>
      
        <!-- Section 3: GO -->
        <div class="hero-section section-3">
            <br>
          <h1 class="hero-header">GO</h1>
        </div>
      </section>
      
      
    

    <section class="featured-carousel">
        <h1 class="section-title">What's the <strong>mood</strong> for the day?</h1>
        <div class="carousel-wrapper">
            <div class="scroll">
                <div class="carousel-track">
                    <div class="carousel-card">
                        <h2>Basic Brew</h2>
                        <img src="images/OrangeAmericano.jpg" alt="Aerocano">
                        <h3>Orange Americano</h3>
                        <br>
                        <p>Rich, creamy, and smooth espresso-based latte.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/ButterscotchLatte.jpg" alt="Butterscotch Latte">
                        <h3>Butterscotch Latte</h3>
                        <br>
                        <p>A perfect blend of espresso, vanilla, and caramel drizzle.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/CheeseAmericano.jpg" alt="Cheese Americano">
                        <h3>Cheese Americano</h3>
                        <br>
                        <p>A bold americano topped with a rich cheese foam.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Non-Coffee</h2>
                        <img src="images/YuriMatchaLatte.jpg" alt="Vienna Latte">
                        <h3>Yuri Matcha Latte</h3>
                        <br>
                        <p>Aromatic espresso with a frothy milk finish.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/IcedMocha.jpg" alt="Iced Mocha">
                        <h3>Iced Mocha</h3>
                        <br>
                        <p>Chocolate and espresso in perfect harmony.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Basic Brew</h2>
                        <img src="images/OrangeAmericano.jpg" alt="Aerocano">
                        <h3>Orange Americano</h3>
                        <br>
                        <p>Rich, creamy, and smooth espresso-based latte.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Basic Brew</h2>
                        <img src="images/IcedCappucino.jpg" alt="Butterscotch Latte">
                        <h3>Iced Cappucino</h3>
                        <br>
                        <p>A perfect blend of espresso, vanilla, and caramel drizzle.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/OrangeMocha.jpg" alt="Cheese Americano">
                        <h3>Orange Mocha</h3>
                        <br>
                        <p>A bold americano topped with a rich cheese foam.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/StrawberryLatte.jpg" alt="Vienna Latte">
                        <h3>Strawberry Latte</h3>
                        <br>
                        <p>Aromatic espresso with a frothy milk finish.</p>
                        <br>
                    </div>
                    <div class="carousel-card">
                        <h2>Artisan Brew</h2>
                        <img src="images/PistachioLatte.jpg" alt="Iced Mocha">
                        <h3>Pistachio Latte</h3>
                        <br>
                        <p>Chocolate and espresso in perfect harmony.</p>
                        <br>
                    </div>
                </div>
            </div>
        </div>
    
        <div class="cta-button-wrapper">
            <a href="products.html" class="cta-button">Mood</a>
        </div>
            </section>
    




    <section class="blend-section-split">
        <!-- LEFT: Scrollable Content -->
        <div class="blend-left scrollable">
          
          <!-- SECTION 1: Membership Perks -->
          <div class="membership-section">
            <h1 class="blend-header">Membership Perks</h1>
            <!-- <h2>📍<em>One Jaya Mall @ Kuching – 2024</em><br>
            📍<em>Plaza Merdeka Shopping Centre @ Kuching – 2025</em></h2> -->

            <ul class="perks-list fancy">
                <li style="--bg: url('../images/INDEXBNG4.png')">
                  🚀 Member Price across all BREW & GO locations
                </li>
                <li style="--bg: url('../images/INDEXBNG10.png')">
                  🌟 Lucky Draw Prizes
                </li>
                <li style="--bg: url('../images/INDEXBNG11REFINED.png')">
                  ☕ Free Drinks for up to 5 Days
                </li>
                <li style="--bg: url('../images/INDEXBNG19.png')">
                  🤩 Collect Points
                </li>
                <li style="--bg: url('../images/INDEXBNG5.png')">
                  📱 Online Ordering - Grab & GO!
                </li>
              </ul>
          </div>
          <!-- SECTION 2: Join Us Info -->
          <div class="membership-section">
            <ul class="tnc-list">
                <p>T&C</p>
              <li>💳 Minimum top-up to join: RM30</li>
              <li>💰 Top-up options: RM50, RM100, RM200</li>
              <li>🔒 Credit stored is not withdrawable</li>
              <li>🕓 Lifetime Membership – never expires</li>
            </ul>
      

          </div>
        </div>

        <!-- RIGHT: Visual Image -->
        <div class="blend-right">
            <div class="right-overlay-content">
              <h1 class="blend-header">Join As Member</h1>
              <hr>
              <img src="images/IndexBNG1.png" alt="Join Our Membership" />
              <div class="button-row">
                <a href="register.html" class="btn">Join Now</a>
                <a href="login.html" class="btn secondary">Already a member? Log In</a>
              </div>
            </div>
          </div>          
      </section>
      
      
    

    <section class="store-locator">
        <!-- Sidebar with Location Selection -->
        <div class="store-sidebar">    
            <!-- Onejaya Selection -->
            <input type="radio" id="onejaya" name="store">
            <label for="onejaya" class="store-item">
                One Jaya Mall
                <p>🕘 9AM - 6PM Daily</p>
            </label>
            <div class="map-wrapper" id="map-onejaya-wrapper">
                <iframe id="map-onejaya"
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3988.366462936843!2d110.3438275!3d1.5566899999999999!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31fba7f35c6fd89b%3A0x16b49ebadfbb7e87!2sOnejaya%20Shopping%20Complex!5e0!3m2!1sen!2smy!4v1710482345678"
                    class="map">
                </iframe>
            </div>
    
            <!-- Plaza Merdeka Selection -->
            <input type="radio" id="plaza-merdeka" name="store" checked>
            <label for="plaza-merdeka" class="store-item">
                Plaza Merdeka
                <p>🕙 10AM - 10PM Daily</p>
            </label>
            <div class="map-wrapper" id="map-plaza-wrapper">
                <iframe id="map-plaza"
                    src="https://maps.google.com/maps?q=Plaza+Merdeka,Kuching,Sarawak&t=&z=15&ie=UTF8&iwloc=B&output=embed" 
                    class="map" allowfullscreen>
                </iframe>
            </div>
        </div>
    </section>

        <section class="barista-section">
            <video class="barista-video" autoplay muted loop>
                <source src="images/Barista2.mp4" type="video/mp4">        
                Your browser does not support the video tag.
            </video>
        
            <div class="barista-image">
                <a href="joinus.html" class="btn join-us-btn"> JOIN US? :3</a>
            </div>
        
            <video class="barista-video" autoplay muted loop>
                <source src="images/Barista1.mp4" type="video/mp4">
                Your browser does not support the video tag.
            </video>
        </section>
    
        <footer class="footer">
            <div class="footer-container">
              <!-- Left Section -->
              <div class="footer-company">
                <a href="#top"><p>Brew & Go Coffee</p></a>
                <p>© COPYRIGHT 2025 BREW & GO</p>
              </div>
    
              <!-- Center Section (Social Media) -->
              <div class="footer-social-links">
                <p><strong>More About Us</strong></p>
                <div class="social-icons">
                  <a href="https://www.instagram.com/brewngo.coffee/" target="_blank"><img src="images/Instagram.png" alt="Instagram"></a>
                  <a href="https://www.facebook.com/profile.php?id=61554234958482" target="_blank"><img src="images/Meta.png" alt="Facebook"></a>
                  <a href="#" target="_blank"><img src="images/Whatsapp.png" alt="Whatsapp"></a>
                  <a href="104404059@students.swinburne.edu.my" target="_blank"><img src="images/Mail.png" alt="Mail"></a>

                </div>
              </div>
          
              <!-- Right Section (Developers) -->
              <div class="footer-developers">
                <p></p>
                <p><a href="profile1.html"> © Terence Wong</a></p>
                <p><a href="profile2.html"> © Darren Chong</a></p>
                <p><a href="profile3.html"> © Hans Yee</a></p>
              </div>
            </div>
          </footer>
          
<!-- Code injected by live-server -->
<script>
	// <![CDATA[  <-- For SVG support
	if ('WebSocket' in window) {
		(function () {
			function refreshCSS() {
				var sheets = [].slice.call(document.getElementsByTagName("link"));
				var head = document.getElementsByTagName("head")[0];
				for (var i = 0; i < sheets.length; ++i) {
					var elem = sheets[i];
					var parent = elem.parentElement || head;
					parent.removeChild(elem);
					var rel = elem.rel;
					if (elem.href && typeof rel != "string" || rel.length == 0 || rel.toLowerCase() == "stylesheet") {
						var url = elem.href.replace(/(&|\?)_cacheOverride=\d+/, '');
						elem.href = url + (url.indexOf('?') >= 0 ? '&' : '?') + '_cacheOverride=' + (new Date().valueOf());
					}
					parent.appendChild(elem);
				}
			}
			var protocol = window.location.protocol === 'http:' ? 'ws://' : 'wss://';
			var address = protocol + window.location.host + window.location.pathname + '/ws';
			var socket = new WebSocket(address);
			socket.onmessage = function (msg) {
				if (msg.data == 'reload') window.location.reload();
				else if (msg.data == 'refreshcss') refreshCSS();
			};
			if (sessionStorage && !sessionStorage.getItem('IsThisFirstTime_Log_From_LiveServer')) {
				console.log('Live reload enabled.');
				sessionStorage.setItem('IsThisFirstTime_Log_From_LiveServer', true);
			}
		})();
	}
	else {
		console.error('Upgrade your browser. This Browser is NOT supported WebSocket for Live-Reloading.');
	}
	// ]]>
</script>
</body>
</html>